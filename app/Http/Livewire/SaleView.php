...
class SaleView extends Component
{
    ...
    public function setData($sale_id)
    {
        ...
        $real_data = $realEstate->city->name . '   بلوك  ' . $realEstate->block_number;
        $real_number_land = $realEstate->land_number;
        ...
        $data_sale = [
            ...
            'real_estate_location_v2' => $real_number_land,
    ...
    public function setPaidAmountData($sale_id)
    {
        ...
        if ($sale->payment_method_id == 2) {
            $payment = "شيك " . $sale->check_number . " " . $sale->bank->name;
        }

        if ($sale->payment_method_id == 3) {
            $payment = "تحويل بنكي (" . $sale->bank->name . ")";
        }
